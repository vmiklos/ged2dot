...
def graph_find(graph, identifier):
    if not identifier:
        return None

    results = [node for node in graph if node.identifier == identifier]
...
class Individual:
    ...
    def resolve(self, graph):
        self.famc = graph_find(graph, self.famc_id)
        for fams_id in self.fams_ids:
            self.fams_list.append(graph_find(graph, fams_id))
...
class Family:
    ...
    def resolve(self, graph):
        self.wife = graph_find(graph, self.wife_id)
        self.husb = graph_find(graph, self.husb_id)
        for child_id in self.child_ids:
            self.child_list.append(graph_find(graph, child_id))
    ...
    with open("test.dot", "w") as stream:
        ...
        for node in subgraph:
            if not node.identifier.startswith("P"):
                continue
            stream.write(node.identifier + " [shape=box, label=\"" + node.surname + " " + node.forename + "\"];\n")
        stream.write("\n")
